...
async def current_user(
    ...
    `request` is here only so the id of whoever is calling can be parked on
    request.state, where the logging middleware picks it up for its one-line
    summary. Nothing else reads it.
    """
    if creds is None:
        raise HTTPException(
    ...
    # Picked up by RequestLoggingMiddleware for the "user=" part of its line.
...
async def require_chairman(user: User = Depends(current_user)) -> User:
    ...
    if user.role != Role.chairman:
        # Worth a line of its own: a member repeatedly hitting a chairman-only
        # route usually means the frontend is showing a button it should hide.
        log.warning("Blocked chairman-only action for user %s (%s)", user.id, user.username)
